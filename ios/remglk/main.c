...
/* On iOS the SwiftUI @main provides the process entry point, so RemGlk's
 * main() is renamed to avoid a duplicate _main symbol at link time.
 * jacl_bridge_run() calls remglk_main() on the terp thread instead. */
int remglk_main(int argc, char *argv[])
#else
