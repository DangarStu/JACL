/* cgi-lib.c - C routines that make writing CGI scripts in C a breeze
   ...
   $Id: cgi-lib.c 1619 2010-02-04 04:12:44Z jacl $
...
int parse_form_encoded(llist* entries)
{
  ...
  cgetline(buffer,BUFSIZ);
  /* now start parsing */
  while ((bytesread=cgetline(buffer,BUFSIZ)) != 0) {
    start = 1;
    ...
    *tempstr = '\0';
    ...
    while (cgetline(buffer, BUFSIZ) > 1) {
    ...
    j = 0;
    while (!done) {
      bytesread = cgetline(buffer,BUFSIZ);
      buffer[bytesread] = '\0';
